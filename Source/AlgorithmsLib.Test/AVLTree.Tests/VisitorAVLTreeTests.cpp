...
namespace Borodin {
	namespace AVLTree {
		...
		TEST_F(AVLTreeFixture, InitializationVisitor123) {
			...
			std::ofstream file("ResultsAVLTreeForGraphviz/Result123.txt");
			std::stringstream ss;
		...
		TEST_F(AVLTreeFixture, InitializationVisitor231) {
			...
			std::ofstream file("ResultsAVLTreeForGraphviz/Result231.txt");
			std::stringstream ss;
			TreePrinter<int> printer(ss);
			printer.PrintInString(first.GetRoot());
			std::string test = ss.str();
		...
		TEST_F(AVLTreeFixture, InitializationVisitor5324971869) {
			...
			std::ofstream file("ResultsAVLTreeForGraphviz/Result5324971869.txt");
			std::stringstream ss;
			TreePrinter<int> printer(ss);
			printer.PrintInString(first.GetRoot());
			std::string test = ss.str();
		...
		TEST_F(AVLTreeFixture, InitializationVisitor_fidsadhysdl) {
			...
			std::ofstream file("ResultsAVLTreeForGraphviz/Result_fidsadhysdl.txt");
			std::stringstream ss;
